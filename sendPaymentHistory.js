...
var nameInputOrg = sheetInput.getRange(lastRowInput, colNumName).getValue(); // 入力された名前(漢字)
var nameKanaInputOrg = sheetInput.getRange(lastRowInput, colNumNameKana).getValue(); // 入力された名前(かな)
...
var mailAddress = sheetInput.getRange(lastRowInput, colNumEmail).getValue();　// Eメールアドレス
  ...
  if (sentmailInput == "yes"){return;} //メール送信欄に"yes"と入力されている場合は処理済みなのでここでScript終了

  if(kaiseiInputOrg.length === 1){var kaiseiInput = "0" + kaiseiInputOrg;} //　回生が1桁の場合は、最初に0を足す
  else{var kaiseiInput = kaiseiInputOrg;}
...
var thisFiscalYear = check_fiscalyear();　//　定義したcheck_fiscalyear関数を呼び出して、現在の会計年度を取得
...
// 検索結果によって異なる処理
  if (arrayMatch.length === 0) { // 検索結果が0件
  ...
  else if (arrayMatch.length === 1) {　//　検索結果が1件
    ...
    for (var i = 0; i <= 2; i++) {
      ...
      if (i === 0){var thisYearPayrec = payrec;} // 今年度の納入情報
      else if (i === 1){var oneYearAgoPayrec = payrec;} // 1年前の納入情報
      else if (i === 2){var twoYearAgoPayrec = payrec;} //　2年前の納入情報
...
var thisMonth = Number(Utilities.formatDate(new Date(), 'JST', 'MM')); // 月を取得
  ...
  // 自動引き落とし希望者へのメッセージを申し込み月によって変更する。１，２，３月に申し込んだ場合、状況によって間に合わないため
...
Logger.log("mailBody: " + mailBody);
  ...
  if (message.indexOf("thebase.in/payments") >= 0) { //クレジットカード支払いの案内をPDFで送るかどうかを、支払いサイトのURLがメール内にあるかどうかで判断。*** 注 *** 2018年4月現在、クレジットカードによる支払いは導入していない    
    ...
    MailApp.sendEmail({      
      to: mailAddress,
      subject: kaiseiInputOrg + "回生 " + nameInputOrg + " 様　" + groupName + " 年会費納入に関するお問い合わせありがとうございました。",
      htmlBody: mailBody,
...
var groupName = configHash["グループ名"];
 ...
 else if (thisYearPayrec == "納入済：銀行口座からの自動引き落とし"){
 ...
 else if (thisYearPayrec == "納入済：" + groupName + "への振込"){ 
 ...
 else if (thisYearPayrec.indexOf("未納入") >= 0){
   var thisMonth = Number(Utilities.formatDate(new Date(), 'JST', 'MM')); // 月を取得
